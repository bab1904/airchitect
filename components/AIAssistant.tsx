...
const AIAssistant: React.FC<AIAssistantProps> = ({ userRole, project }) => {
  ...
  return (
    <>
      ...
      >
         {isOpen ? <X color="white" /> : <HardHat color="white" />}
      </button>

      {/* Chat Window */}
      {isOpen && (
        <div className="fixed bottom-24 right-6 w-96 h-[600px] bg-white rounded-2xl shadow-2xl border border-slate-200 flex flex-col z-40 overflow-hidden animate-fade-in-up">
            <div className="bg-indigo-600 p-4 text-white flex items-center gap-2">
                <Bot size={20} />
                <div>
                    <h3 className="font-semibold leading-tight">Construction Advisor</h3>
                    <p className="text-xs opacity-80 font-light">Virtual Civil Engineer</p>
                </div>
            </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-4">
                        {suggestions.map((s, idx) => (
                            <button 
                                ...
                                className="bg-white border border-slate-200 p-2 rounded-lg text-xs text-left hover:bg-indigo-50 hover:border-indigo-200 transition-colors flex flex-col gap-1 shadow-sm"
                            >
            ...
            <div className="p-3 bg-white border-t border-slate-200">
                <div className="flex gap-2">
                    <button onClick={handleVoiceInput} className="p-2 text-slate-400 hover:bg-slate-100 rounded-full transition-colors">
                        <Mic size={20} />
                    </button>
                    <input 
                        type="text" 
                        ...
                        className="flex-1 bg-white text-slate-900 border border-slate-200 rounded-full px-4 py-2 focus:ring-2 focus:ring-indigo-500 outline-none text-sm placeholder-slate-400"
                    />
                    <button onClick={() => handleSend()} disabled={!input.trim()} className="p-2 text-indigo-600 hover:bg-indigo-50 rounded-full transition-colors disabled:opacity-50">
                        <Send size={20} />
                    </button>
                </div>
            </div>
